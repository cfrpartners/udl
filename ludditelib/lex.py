...
class Lexer:
    def __init__(self):
        ...
        self.lexindexfunc = []
            # Reverse mapping of groups to functions and types
    ...
    def realtoken(self):
        ...
        while lexpos < lexlen:
            ...
            if self.lexerrorf:
                ...
                if oldpos == lexpos:
                    # Error method didn't change text position at all. This is
                    # an error.
                    self.lexpos = lexpos
...
def lex(module=None, debug=0, optimize=0, lextab="lextab", reflags=0):
    ...
    ssymbols = []
    for f in tsymbols:
        if isinstance(ldict[f], collections.Callable):
    ...
    fsymbols.sort(lambda x, y: cmp(
        x.__code__.co_firstlineno, y.__code__.co_firstlineno))

    # Sort the strings by regular expression length
    ssymbols.sort(lambda x, y: (len(x[
                  1]) < len(y[1])) - (len(x[1]) > len(y[1])))
    ...
    for name, r in ssymbols:
        ...
        if not optimize:
            if name == 't_error':
                raise SyntaxError(
                    "lex: Rule 't_error' must be defined as a function")
                error = 1
                continue
